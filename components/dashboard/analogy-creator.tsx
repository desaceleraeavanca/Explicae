...
export function AnalogyCreator({ audiences = [] }: AnalogyCreatorProps) {
  // Garantir que audiences seja sempre um array válido
  ...
  // Garantir que analogies seja sempre um array válido
  const safeAnalogies = Array.isArray(analogies) ? analogies : []

  const handleAudienceSelect = (audienceId: string) => {
    setSelectedAudienceId(audienceId)
    const selected = safeAudiences.find((a) => a.id === audienceId)
    if (selected) {
  ...
  const generateAnalogies = async () => {
    ...
    try {
      ...
      // A API retorna 'analogies' com objetos que têm 'titulo' e 'descricao'
      // Precisamos mapear para 'title' e 'description'
      const analogiesData = data.analogies || data.analogias || []
      const mappedAnalogies = analogiesData.map((analogy: any) => ({
        title: analogy.titulo || analogy.title || '',
        description: analogy.descricao || analogy.description || ''
      }))
      setAnalogies(mappedAnalogies)
      if (data.usage) {
        setUsage(data.usage)
      }
    ...
    <div className="space-y-8">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {safeAudiences && safeAudiences.length > 0 && (
            <div className="space-y-2">
